...
public class AdminMemberController {
    ...
    public ResponseEntity<?> suspend(@PathVariable Integer id) {
        return memberRepository.findById(id).map(m -> {
            if ("ROLE_ADMIN".equals(m.getRole())) {
                return ResponseEntity.badRequest().<Object>body("관리자 계정은 정지할 수 없습니다.");
            }
    ...
    @PatchMapping("/{id}/expel")
    public ResponseEntity<?> expel(@PathVariable Integer id) {
        return memberRepository.findById(id).map(m -> {
            if ("ROLE_ADMIN".equals(m.getRole())) {
                return ResponseEntity.badRequest().<Object>body("관리자 계정은 강퇴할 수 없습니다.");
            }
